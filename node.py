...
class BlockChain:
    def __init__(self):
        self.adjust_difficulty_blocks = 10    #每多少個區塊調節一次難度
        self.difficulty = 1    #目前難度
        self.block_time = 30    #理想上多久能夠出一個區塊
    ...
    def mine_block(self, miner):    #挖礦的主函式
        ...
        while new_block.hash[0: self.difficulty] != '0' * self.difficulty:    #若至少有 self.difficulty 個 0，說明已經找到 nonce
            new_block.nonce += 1    #怎麼調整nonce有沒有差?
            new_block.hash = self.get_hash(new_block, new_block.nonce)
    ...
    def get_balance(self, account):    #檢查 匯款人account 的餘額是否足夠
        balance = 0    #該帳戶的餘額
    ...
    def receive_socket_message(self, connection, address):    #接收訊息後處理
        with connection:
            ...
            while True:
                ...
                if message: 
                    if parsed_message["request"] == "get_balance":    #使用者想要做取得帳戶餘額
                    ...
                    elif parsed_message["request"] == "transaction":    #使用者想要發起交易
